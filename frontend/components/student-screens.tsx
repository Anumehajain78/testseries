...
export function StudentExamScreen() {
  const params = useParams<{ id: string }>(); const router = useRouter(); const { state, hydrated, currentStudentId, answerQuestion, flagQuestion, submitExam } = useExam(); const [current, setCurrent] = useState(0); const [dialog, setDialog] = useState(false); const test = state.tests.find((t) => t.id === params.id); const submission = state.submissions.find((s) => s.testId === params.id && s.studentId === currentStudentId); const finish = useCallback((mode: "manual" | "automatic") => { submitExam(params.id, mode); router.replace("/student/submitted"); }, [params.id, submitExam, router]); const autoSubmit = useCallback(() => finish("automatic"), [finish]);
